...
import { updatePassword, verifyToken } from "@/actions/reset-passwords";
...
export default function ResetPassword({ token }: { token: string }) {
  ...
  useEffect(() => {
    const verifyTokens = async () => {
      const { status, message, user } = await verifyToken(token)
      if (status === 200) {
        setUser({
          ...user,
          email: user.email,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { status, message } = await updatePassword(user)
    if (status === 200) {
      toast.success(message);
      setUser({ email: "", newPassword: "", ReNewPassword: "" });
  ...
  useEffect(() => {
    if (user?.newPassword?.length >= 0 && user?.ReNewPassword?.length >= 0) {
      setDisable(false);
    } else {
      setDisable(true);
    }
  ...
  return (
    <div className="bg-white dark:bg-gray-dark dark:shadow-card rounded-lg p-6 w-full max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-5">
          <button
            type="submit"
            className={`w-full cursor-pointer rounded-lg ${disable ? "bg-[#428b7196]" : "bg-primary"} p-4 font-medium text-white transition hover:bg-opacity-90`}
            disabled={disable}
          >
            Create Password
          </button>
