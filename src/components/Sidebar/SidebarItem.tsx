...
const SidebarItem = ({ item, pageName, setPageName }: any) => {
  ...
  return (
    <>
      {
        item.permission ?
          <PermissionCheck permission={item.permission}>
            <SidebarList item={item} handleClick={handleClick} isItemActive={isItemActive} pageName={pageName} />
          </PermissionCheck>
          :
          <SidebarList item={item} handleClick={handleClick} isItemActive={isItemActive} pageName={pageName} />
      }
...
const SidebarList = ({ item, handleClick, isItemActive, pageName }: any) => {
  const handleLinkClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    // If route is "#" or has onClick handler, prevent navigation
    if (item.route === "#" || item.onClick) {
      e.preventDefault();
      if (item.onClick) {
        item.onClick();
      }
      // Only toggle dropdown state, don't navigate
      handleClick();
      return;
    }
    // For normal navigation, just update pageName state
    handleClick();
  };
    ...
    <li>
      {item.route === "#" ? (
        <button
          ...
          className={`${isItemActive ? "bg-primary/[.07] text-primary dark:bg-white/10 dark:text-white" : "text-dark-4 hover:bg-gray-2 hover:text-dark dark:text-gray-5 dark:hover:bg-white/10 dark:hover:text-white"} group relative flex w-full items-center gap-3 rounded-[7px] px-3.5 py-3 font-medium duration-300 ease-in-out`}
        >
          {item.icon}
          {item.label}
          {item.message && (
